#!/usr/bin/env python2
def parse_file(x, num_tasks, lambda_decre, comb_list, run_input):
    """ description: Parse molaris output file to extract the potential energy of 
                      ...
                      "randomly selected replicas" are allowed or not, based on the metropolis
                      hastings criterion. This random swaps are tried every 5 replica swap.
                      This random swap is to introduce better sampling.

                      1. If the calculated boltzmann probability is less then one, then
             ...
             comb_list: list of preselected random pairs of replica windows
                        the selection is based on random integer pairing
          ...
          '2zf0_ac_run.inp' for this script.
           In future this script would replace the parser.py script"""
...
#          comb_list = gen_pair.gen_pair(initial, final-2)
          ...
          for i, j in comb_list:
              delta=-(epot_original[i]+epot_original[j]-epot_swap[i]-epot_swap[j])/RT
              accepted=False
    #          print delta 
              
              if delta<0:
                 ini_random_switch.random_sw(i, j, lambda_decre, run_input, sys)
                 accepted=True
                 print('Unconditionally accepted [Delta]:',delta, "exchange between", i, "and", j, "\n")
              elif delta<11.0:
                   p=math.exp(-delta)
                   rnd=np.random.rand()
                   if p>rnd:
                      ini_random_switch.random_sw(i, j, lambda_decre, run_input, sys)
                      accepted=True
                      print('Conditionally accepted [P,r]:',p,rnd , "..", "exchange between", i, "and", j, "\n")
                   else:
                       no_switch.no_sw(i, j, lambda_decre, run_input)
                       print('No acceptance [P, r]:', p, rnd, "..", "exchange attempted", i, "and", j, "\n")
       else:
           sys.exit(1)
